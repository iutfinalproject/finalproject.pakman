...
background::background(int &select_page,QObject *parent, QGraphicsScene* nowscene, int nowid) :
    QObject(parent),select_page(select_page)
{
...
void background::addplayer()
{
    ...
    player_1 *player1_;
       player1_=new player_1(desktopWidth,desktopHight);
       player1_->setFlag(QGraphicsItem::ItemIsFocusable);
       player1_->setFocus();
       scene->addItem(player1_);
     player_2 *player2_;
          player2_=new player_2(desktopWidth,desktopHight);
